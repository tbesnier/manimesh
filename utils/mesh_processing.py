...
def create_gif_shape(list_data, faces, file_dir, file_name, x_view=0, y_view=0, z_view=0, auto_scale = True):
    filenames = []
    listpq = list_data
    steps = []   
    faces = faces
...
def create_gif_shape_inconsistent(list_data, list_faces, file_dir, file_name, x_view=0, y_view=0, z_view=0, auto_scale = True):
    filenames = []
    listpq = list_data
    steps = []   
    for i in range(len(listpq)):
    ...
    for i in range(len(listpq)):
        ...
        V = np.array(steps[i])
        #V = np.array([it[:,0], it[:,1], it[:,2]]).T
        F = np.array([list_faces[i][:,0], list_faces[i][:,1], list_faces[i][:,2]]).T
